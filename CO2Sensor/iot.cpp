...
bool IOT::publishBin(String topic, const uint8_t *message, unsigned int len)
{
    while (!_pubsub_client->connected())
    {
        _pubsub_client->connect(DEVICE_NAME);
...
bool IOT::publish(String topic, char *message)
{
    while (!_pubsub_client->connected())
    {
        _pubsub_client->connect(DEVICE_NAME);
    ...
    return _pubsub_client->publish(topic.c_str(), message);
}
